function columns(schema, connection) {
    return `
SELECT
    tab.TABLE_NAME,
    col.COLUMN_NAME,
...
-- chosen schema
WHERE tab.TABLE_SCHEMA = ${connection.escape(schema)}
...
function manyToMany(schema, connection) {
    return `
SELECT
...
WHERE ke.REFERENCED_TABLE_SCHEMA = ${connection.escape(schema)}
...
function manyToOne(schema, connection) {
    return `
SELECT
...
function oneToOne(schema, connection) {
    return `
SELECT
  ke.TABLE_NAME as 'TABLE_FROM',
  ke.COLUMN_NAME as 'COL_FROM',
