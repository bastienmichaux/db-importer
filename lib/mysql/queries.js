...
/* returns the query used to retrieve all columns for the selected tables */
const allColumns = (schema, tables) => {
    if (typeof schema !== 'string') {
        throw new TypeError(`Expected 'schema' parameter to be a string, was ${typeof schema} instead.\nValue: ${schema}`);
    }
    if (typeof tables !== 'string') {
        throw new TypeError(`Expected 'tables' parameter to be a string, was ${typeof tables} instead.\nValue: ${tables}`);
    }

    const tablesCondition = (tables ? `AND col.table_name IN (${tables})` : '');

    return `SELECT col.table_name, col.column_name
FROM information_schema.columns col
WHERE col.table_schema LIKE ${schema}
${tablesCondition};`;
};
