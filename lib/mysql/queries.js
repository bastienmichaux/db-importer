...
const tables = (schema, filter) => {
    // if filter is an empty string, do not include the last part of the query
    const queryFilter = (filter ? `AND tab.TABLE_NAME NOT IN (${escape(filter)})` : '');
...
const manyToManyJunction = (schema, tables) => {
     ...
     */
    const conditionTables = tables ? `AND ke.TABLE_NAME NOT IN (${escape(tables)})` : '';
    const conditionReferencedTables = tables ? `AND ke.REFERENCED_TABLE_NAME IN (${escape(tables)})` : '';

    return `SELECT
ke.TABLE_NAME AS 'JUNCTION_TABLE',
GROUP_CONCAT(ke.COLUMN_NAME ORDER BY ke.REFERENCED_TABLE_NAME ASC) AS 'FOREIGN_KEYS',
GROUP_CONCAT(col.COLUMN_TYPE ORDER BY ke.REFERENCED_TABLE_NAME ASC) AS 'REFERENCED_TABLES',
...
const columns = (schema, tables) => {
    const tablesCondition = (tables ? `AND col.table_name IN (${escape(tables)})` : '');
