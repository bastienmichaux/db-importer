...
/**
 * set a commonCode property to the received error object depending his error.code
 * this is so the code is not dbms specific
 *
 * @param {Error} error to set her commonCode
 */
const setCommonCode = (error) => {
    switch (error.code) {
    case cst.DB_ERRORS.HOST_UNREACH:
        error.commonCode = db.DB_ERRORS.HOST_UNREACH;
        break;
    case cst.DB_ERRORS.ACCESS_DENIED:
        error.commonCode = db.DB_ERRORS.ACCESS_DENIED;
        break;
    default:
        error.commonCode = db.DB_ERRORS.UNHANDLED;
    }
};
