...
const entitiesTables = (session) => {
    ...
    return promisedQuery(db.FIELDS.jhipster, jhipsterQuery)
        ...
        .then(() => {
            ...
            const manyToManyTablesOnly = queries.manyToManyTablesOnly(session.schema, filter);

            return promisedQuery(db.FIELDS.manyToManyTablesOnly, manyToManyTablesOnly);
        })
        .then(() => {
            // filter tables we don't want to query
            const filter = lodash.flatten(lodash.values(session.results));
 ...
 *
 * @param { Object[] } queryResults - an array of RowDataPacket objects, the output of queries.columns (see .entityCandidatesColumns)
...
const manyToManyJunctions = (session) => {
    ...
    return promisedQuery(db.FIELDS.manyToManyJunctions, manyToManyQuery)
        .then(() => session);
};
