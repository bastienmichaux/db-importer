...
const promisedQuery = (session, query, store, etl) => new Promise((resolve, reject) => {
    ...
    session.connection.query(query, (error, results) => {
        ...
        session.results[store] = etl ? results.map(etl) : results;
...
const entitiesTables = (session) => {
    ...
    const etl = tableObject => tableObject.TABLE_NAME;
...
const manyToManyJunctions = (session) => {
    ...
    return promisedQuery(session, manyToManyQuery, db.FIELDS.manyToManyJunctions, etl)
        .then(() => session);
};
