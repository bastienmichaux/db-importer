...
const connect = (session) => {
    ...
    session.connection.promisedQuery = (query, parameters) => new Promise((resolve, reject) => {
        session.connection.query(query, parameters, (error, results) => {
...
const entityCandidates = session => session.connection.promisedQuery(cst.queries.jhipster, { database: session.database })
    .then((results) => {
        session.results = {
            jhipster: lodash.map(results, cst.fields.tableName)
        };
    })
    .then(() => session.connection.promisedQuery(cst.queries.liquibase, { database: session.database }))
    .then((results) => {
