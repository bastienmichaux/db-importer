...
/**
 * Get the list of tables at the exclusion of junction tables between two tables
 *
 * That is because such table will end up as a relationship and not an entity
 * @param session an object containing the connection object and the driver used to get the tables
 * @returns {Promise.<{}>} the same session object completed with the array
 */
const entityCandidates = session => session.driver.entityCandidates(session.connection, session.driver) // todo not sure it must be a promise
    .then(() => session);
