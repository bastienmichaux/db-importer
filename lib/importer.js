...
const getMysqlDumpAsXmlCommand = (databaseName, user) => `mysqldump -u ${user} -p -d -X ${databaseName}`;

const getMysqlDumpAsXml = (credentials) => {
    // const command = `${getMysqlDumpAsXmlCommand(credentials.database, credentials.username)} > ../dump/${credentials.database}.xml`;
    // const child = spawn(command);
    const destination = `../dump/${credentials.database}.xml`;
    const command = spawn('mysqldump', ['-u', `${credentials.username}`, '-p', '-d', '-X', `${credentials.database}`]);
    command.stdout.pipe(destination);
};
...
const getAllMysqlTables = (credentials, tables) => {
    ...
    tables.forEach((table) => {
        if (typeof table === 'string') {
            ...
            // get the sql query describing the database
            query = `describe ${table};`;

            // get the path of the file where we will write the description as JSON
            filePath = getFilePath(table);

            // pass the query to the sql driver,
            // then process the output to get a proper JS object
            ...
            .then(
                (queryOutput) => {
                    if (Array.isArray(queryOutput)) {
                        queryOutput.forEach((elem) => {
                            if (Array.isArray(elem)) {
                                elem.forEach((_elem) => {
                                    if (_elem.constructor.name === 'RowDataPacket') {
                                        let field = _elem.Field; // eslint-disable-line prefer-const
                                        result[field] = {
                                            type: _elem.Type,
                                            null: _elem.Null,
                                            key: _elem.Key,
                    ...
                    fs.writeFileSync(getFilePath(table), fileContent, 'utf-8');
                },
                (onRejected) => {
